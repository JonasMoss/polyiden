...
#Part 1: Computing the Spearman correlation when assuming a 
#Gaussian copula.
#
#To find the normal-theory Spearman correlation:
#rhohat is an estimate of the parameter of the gaussian copula.
#Spearman's correlation is the correlation of this copula when
#paired with uniform marginals.
#We approximate this correlation via numerical integration.

spearmanGaussCop <- function(p) {
  rhohat <- polycor::polychor(p)
  norm.cop <- normalCopula(rhohat)
  varUnif <- (1/12)
  ...
  CNorm <- function(u,v) {
    x <- c(u,v)
    return(pCopula(x, norm.cop))
  }
  return(CovIntUnif(CNorm)$integral)
}

##Part 2: Implementations of the bounds from Proposition 1 and 2.
#Please see the sections in the supplementary online material 
#that explains the simplifications we use to compute the integrals.

#Function "lcorr". Gives bounds on latent correlations with fixed marginal distributions
#Assumed that the marginal distributions are equal, and that they support
