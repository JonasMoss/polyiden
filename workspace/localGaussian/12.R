#One correlation per category, following the diagonal

#Did not work, since there is not always a Gaussian which fulfil this property.
...
#Do we have uniqueness from this shape?
for(i in (1:K)) {
  print(i)
  i <- 4
  diffCDF = Vectorize(function(rhoCand) {
    if(i == 1) {
      prob <-  pCopula(c(pnorm(tau1[i]),pnorm(tau2[i])), normalCopula(param=rhoCand, dim = 2))
    } else {
      prob <-  pCopula(c(pnorm(tau1[i]),pnorm(tau2[i])), normalCopula(param=rhoCand, dim = 2)) - pCopula(c(pnorm(tau1[i-1]),pnorm(tau2[i-1])), normalCopula(param=rhoCand, dim = 2))
    }
    return(P$CDF[i] - prob)
  })
  rho = seq(-1, 1, by = 0.01)
  plot(rho, diffCDF(rho), type = "l")
