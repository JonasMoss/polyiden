...
for(k in (1:dim(S)[1])) {
  ...
  #new idea, choose the solution closest to the "tetrachoric" solution
  ...
  if(length(rootsAll) == 0) {
    ...
    if(length(rootsAll2) > 1) {
      for(i in (1:length(rootsAll2))) {
        PRoots <- rbind(PRoots, c(k, rootsAll2[i], optRes$objective))
      } 
    } else {
...
#    abline(h=0)
    ...
    for(i in (1:length(rootsAll))) {
      PRoots <- rbind(PRoots, c(k, rootsAll[i], NA))
    }
  }
...
#This solution has the property of minimizing the variation
#of the solutions.
#Could use this as a criterion? Though then it is possible
#that if e.g. the upper corner and lower corner actually change

#Could choose the solution which is closest to the "tetrachoric" solution?
