...
tetrachoricBox <- function(k.ind, monotone=FALSE) { #Finds the distributions which match up with the boxes with
  ...
  epsInterval <-  c(-1 + 0.001 ,1-  0.001)  #numerical problems appear very close to -1,1
  
  res <- uniroot.all(probDiff,epsInterval)
  if(length(res) == 0) {
    res <- optimize(function(rhoCand) { abs(probDiff(rhoCand)) }, epsInterval)$minimum
  } 
  if ((length(res) > 1) & (monotone == TRUE)) {
    warning(paste("More than one solution in a tetrachoric problem for k.ind=", k.ind, ". Likely non-separated solution", sep=""))
    plot(seq(-0.999, 0.999, by = 0.01), probDiff(seq(-0.999, 0.999, by = 0.01)), type = "l", xlim=c(-1,1), main=paste("k=", k.ind,sep="", collapse = ","))
    abline(h=0)
    #There is only one solution since the criterion function is monotone.
    #Likely this happens if the criterion function is almost flat near -1 or 1.
    #Then the correct solution is the one with the maximum absolute value.
...
for(k in (1:dim(S)[1])) {
  ...
  if(length(rootsAll) == 0) { #no "exact" solution, so we use the best available.
    ...
    if(which.max(gathered) == 1) {
      #NE
      tetrachoric <- tetrachoricBox(k.ind.ne, monotone = TRUE)
    } else if(which.max(gathered) == 2) {
      #NW 
      tetrachoric <- tetrachoricBox(k.ind.nw, monotone = TRUE)
    } else if(which.max(gathered) == 3) {
      #SE 
      tetrachoric <- tetrachoricBox(k.ind.se, monotone = TRUE)
    } else {
      #SW
      tetrachoric <- tetrachoricBox(k.ind.sw, monotone = TRUE)
    }
...
for(k in (1:length(P$i) )) {
  ...
  text(xlim, ylim,  round(P$rhoLocal[k],2), cex=1.3)
...
copObjmvdc <- mvdc(claytonCopula(theta), c("norm", "norm"),list(list(mean = 0, sd =1), list(mean = 0,sd=1)))
contour(copObjmvdc, dMvdc, xlim=c(-3, 3), ylim=c(-3, 3), drawlabels=F, add=T, lty=2)
