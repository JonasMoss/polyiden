...
tetrachoricBox <- function(k.ind) { #Finds the distributions which match up with the boxes with
  ...
  print(k.ind)
  plot(seq(-1, 1, by = 0.01), probDiff(seq(-1, 1, by = 0.01)), type = "l", xlim=c(-1,1), main=paste("k=", k.ind,sep="", collapse = ","))
  abline(h=0)
...
for(k in (1:dim(S)[1])) {
  ...
  #For the four corners of the square, there is only one possible tetrachoric correlation, which already
  #corresponds to the solution found. The reason for this is that all other directions contain only
  #marginal information (which is fixed, and constant for all correlations)
  if( !(((S[k,1] == 1) & (S[k,2] == 1)) |
        ((S[k,1] == K) & (S[k,2] == K)) |
        ((S[k,1] == 1) & (S[k,2] == K)) |
        ((S[k,1] == K) & (S[k,2] == 1))
    ...
    #If we include all boxes except one row or column, the probability is given by marginal information.
    #We discart such cases.
    if(ne.prob + sum(P$Box[-k.ind.ne]) >= 1-(10*.Machine$double.eps)) {
      ne.prob <- 0 #deducible from marginal information
    }
    if(nw.prob + sum(P$Box[-k.ind.nw]) >= 1-(10*.Machine$double.eps)) {
      nw.prob <- 0 
    }
    if(se.prob + sum(P$Box[-k.ind.se]) >= 1-(10*.Machine$double.eps)) {
      se.prob <- 0 
    }
    if(sw.prob + sum(P$Box[-k.ind.sw]) >= 1-(10*.Machine$double.eps)) {
      sw.prob <- 0 
    }
