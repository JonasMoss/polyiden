...
for(k in (1:dim(S)[1])) {
  ...
  #We treat all cases equally, even the corners where we have just one "tetrachoric" solution,
  #which also will correspond to the local solution just computed.
  ...
  #we discard trivial soltuions, e.g. 0 or 1.
  ...
  if(which.max(gathered) == 1) {
    #NE
    tetrachoric <- findTetra(k.ind.ne)
  } else if(which.max(gathered) == 2) {
    #NW 
    tetrachoric <- findTetra(k.ind.nw)
  } else if(which.max(gathered) == 3) {
    #SE 
    tetrachoric <- findTetra(k.ind.se)
  } else 
    #SW
    tetrachoric <- findTetra(k.ind.sw)
  }

  #nb, which.max always returns just one element, so this will work also if max is not unique.
  
#We find the direction which are left that has the maximum probability (other alternatives
  #coudl be used instead of the maximum, which still preserves invariance towards "rotations", such as
  #the mean of the correlation directions, though this is the combination of several correlations,
  #and not the correlation of a single distribution over a square)
  ...
  } else if ( ( (S[k,1] == K) & (S[k,2] == 1)) ||  ((S[k,1] == K) & (S[k,2] == K) ) ) {
    #for the top left and right corner, we can just match the box probabilities, as these
    #are infinite expanding squares. It should always be unique.
    #We therefore do nothing here. 
    best.inx <- 1
    
    if(length(rootsAll) > 1) {
      stop("Invalid probability table") #Should have a general check on whether the probability
      #table is invalid.
    }
    
    
  } else {
    #We here match survival copulas instead.
