...
getBoundsNormalBase <- function(K, rho) {
  ...
  #The thresholds include 1, corresponding to the final threshold \tau_{i,K} which is always \infty for normal marginals, but we do not not include zero, which corresponds to \tau_{i,0} = -\infty, which is not needed here.

  #deterministic thresholds, with intended non-symmetry
  tau1 <- c(qnorm(seq(0.05, 0.99, length.out=K-1)),Inf)  #USED TO BE 1 not INF!
  tau2 <- c(qnorm(seq(0.01, 0.97, length.out=K-1)), Inf)#USED TO BE 1 not INF!
...
#  abline(a=0,b=1)
  ...
  end_time <- Sys.time()
  res <- c(K, lower, upper, lowerSym, upperSym, end_time - start_time)
