...
#For arbitrary positive correlation, we need one point at the (1,1) box, diverging to infinity.
...
#This script only considers the positive case, which is a straight forward modification of the code I
...
#check that everything is OK also for independence:
#cvCop[[1]][[1]]$parameters[1,1] <- 0
#theta <- as.numeric(cvCop[[1]][[1]]$parameters)
#worked well also here.
...
#works. Though of course, only if the (0,0) has non-zero probability.
#Would be much more elegant if this could be done for any box, as we
#can then go directly to the infinite case, and that this will always work for any table.