...
#could re-set this parameter to rho, as we are here in a gaussian setting, where the correspondence is the identity function
...
getPointsNegCor <- function(P, M, eps = 0.001) {
  ...
  numPoints <- 50
  #upperLeftLim <- c(-M, yorig)
  upperLeftLim <- c(-M, -M/2)
  #lowerRightLim <- c((xorig-M)/2,-M)
  #lowerRightLim <- c(xorig,-M)
  lowerRightLim <- c(xorig,-M)
  
  probLinePoints <- XYprob[1,3]/numPoints
  
  lineModX <- seq(upperLeftLim[1], lowerRightLim[1], length.out = numPoints)
  ...
  b <- deltaY/deltaX
  a <- (-y1*x2+y2*x1)/(x1-x2)
  lineModY <- a + b*lineModX
  #points(lineModX, lineModY)
  
  XYprobMod <- rbind(cbind(lineModX, lineModY, probLinePoints), as.matrix(XYprob[-1,]))
  XYprobMod <- as.data.frame(XYprobMod)
  colnames(XYprobMod) <- c("x","y","prob")
  return(XYprobMod)
}
...
#the hard-coded construction only works for M = 0 and sufficiently large M.
drawPoints(P, M=0) 
drawPoints(P, M=5)
drawPoints(P, M=10)
drawPoints(P, M=100)
